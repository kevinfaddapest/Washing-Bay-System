...
if ($_SESSION['role'] == 'staff') {
    $where[] = "username='" . $conn->real_escape_string($_SESSION['user']) . "'";
}

if ($search != '') {
    $s = $conn->real_escape_string($search);
    $where[] = "(username LIKE '%$s%' OR action LIKE '%$s%' OR details LIKE '%$s%' OR ip_address LIKE '%$s%' OR date_time LIKE '%$s%')";
}
...
// ✅ Calculate total records (no sum needed here)
