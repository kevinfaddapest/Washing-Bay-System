...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($amount >= 25000) {
        ...
        if (!isset($_FILES['receipt_file']) || $_FILES['receipt_file']['error'] !== UPLOAD_ERR_OK) {
            $error = "Receipt file is required for expenses 25,000 UGX or more.";
        } else {
            ...
            if (!in_array($_FILES['receipt_file']['type'], $allowed_types)) {
                ...
                $receipt_file_name = time() . '_' . basename($_FILES['receipt_file']['name']);
                move_uploaded_file($_FILES['receipt_file']['tmp_name'], $upload_dir . $receipt_file_name);
            }
        }
    }
    ...
    if (!isset($error)) {
        $stmt = $conn->prepare("
            INSERT INTO expenses (expense_name, category, amount, date, added_by, receipt_number, receipt_file) 
            VALUES (?, ?, ?, ?, ?, ?, ?)
        ");

        // Bind parameters, use NULL for receipt fields if empty
        $stmt->bind_param(
            "ssdssss",
            $expense_name,
            $category,
        ...
        if ($stmt->execute()) {
            ...
            if ($amount >= 25000) $details .= ", Receipt Number: $receipt_number, File: $receipt_file_name";
